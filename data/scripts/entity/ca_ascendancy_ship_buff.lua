...
function updateServer(timeStep)
    ...
    entity:addMultiplyableFactor(StatsBonuses.ShieldCapacity, 0)
    entity:removeMultiplyableFactor(StatsBonuses.ShieldCapacity)
end
...
function onBaseMultiplierCalculated(entity, statModifier)
    ...
    if tier > 0 then
        ...
        -- Avorion has StatsBonuses.BaseDamageMultiplier ? No, let's use ShieldCapacity and Velocity to be safe,
        -- as modifying raw damage via base multipliers isn't always reliable.
        -- Actually we can boost ShieldCapacity and HyperspaceCooldown!
        ...
        statModifier:modifyBaseMultiplier(StatsBonuses.ShieldCapacity, shieldMult)
